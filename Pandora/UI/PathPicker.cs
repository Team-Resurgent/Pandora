...
namespace Pandora
{
    public class PathPicker
    {
        ...
        public bool Render()
        {
            ...
            if (m_inputDialog.Render() && m_inputDialog.Cancelled == false)
            {
                try
                {
                    Directory.CreateDirectory(Path.Combine(SelectedFolder, m_inputDialog.Input));
                }
                catch
                {
                    // ignore
            ...
            var disableCreate = !Directory.Exists(SelectedFolder);
            if (disableCreate)
            {
                ImGui.BeginDisabled();
            }
            if (ImGui.Button("Create Folder", new Vector2(100, 30)))
            {
                m_inputDialog.Title = "Create Folder";
                m_inputDialog.Message = "Enter folder name.";
                m_inputDialog.ShowModal();
            }
            if (disableCreate)
            {
                ImGui.EndDisabled();
